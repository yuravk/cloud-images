...
source "qemu" "almalinux_kitten_10_vagrant_vmware_qemu_aarch64" {
  ...
  net_device         = "virtio-net"
  qemu_binary        = var.qemu_binary
  vm_name            = "almalinux_kitten_10_vagrant_vmware_qemu_aarch64.raw"
...
build {
  ...
  post-processors {
    ...
    post-processor "artifice" {
      files = [
        "AlmaLinux-Kitten-Vagrant-vmware-10-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.aarch64.box",
        "AlmaLinux-Kitten-Vagrant-vmware-10-${formatdate("YYYYMMDD", timestamp())}.${var.build_number}.aarch64.raw",
      ]
      only = ["qemu.almalinux_kitten_10_vagrant_vmware_qemu_aarch64"]
    }
  }
