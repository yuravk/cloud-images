...
| `s390x` | `false` | Also build the s390x image under QEMU TCG emulation on an x86_64 runner (experimental, hours per run; offline validation only). See [s390x under TCG](#s390x-under-tcg-experimental). |
...
init-data
 ...
 '- build-s390x-tcg (opt-in, s390x=true)                 -  shared-steps build under
...
speed - TCG is roughly 10-20x slower than native, so expect hours per
image (the job allows 12). The leg is opt-in (`s390x=true`) so scheduled
builds are unaffected until it has proven itself.
